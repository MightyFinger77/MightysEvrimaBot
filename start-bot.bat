...
if not exist "evrima-server-bot-1.2.0.jar" (
    echo [ERROR] evrima-server-bot-1.2.0.jar not found in this folder.
    echo Put start-bot.bat next to the shaded JAR from mvn package.
    pause
    exit /b 1
)

for %%I in ("evrima-server-bot-1.2.0.jar") do set "JARSIZE=%%~zI"
if %JARSIZE% LSS 5000000 (
    ...
    echo Then copy THIS file to EvrimaBot ^(not "original-evrima-server-bot-1.2.0.jar"^):
    echo   target\evrima-server-bot-1.2.0.jar   ^(~35 MB with dependencies^)
    pause
    exit /b 1
)
...
java %JVM_EXTRA% -jar "evrima-server-bot-1.2.0.jar"
set "ERR=%ERRORLEVEL%"
