...
**Channel topic (server status line):** optional `server_status_topic` sets **Discord channel topic(s)** on a timer (e.g. `12/60 players online | 847 unique players seen | Last update: …`). Use **`channel_id`** for one channel or **`channel_ids`** (YAML list) for several—the same text is applied to each (e.g. population dashboard channel and in-game chat channel). Both keys can be set together; IDs are merged and deduplicated. The bot needs **Manage Channels** on every channel listed. **Unique players** are Steam IDs ever seen in RCON `playerlist` while the bot runs (stored in SQLite), not the game’s full historical join total. Evrima RCON does not report dedicated-server uptime; set `show_bridge_uptime: true` only if you want **JVM / bridge** uptime as a rough line item. Use `max_players` for the cap shown after `/`, or `0` to omit the cap.
