#!/usr/bin/env bash
# EvrimaServerBot 1.0.2 launcher for Linux (and macOS). Keep this script next to the shaded JAR from: mvn package
# First run: chmod +x start-bot.sh

set -euo pipefail

cd "$(dirname "$0")"
JAR="evrima-server-bot-1.0.2.jar"

if [[ ! -f "$JAR" ]]; then
  echo "[ERROR] $JAR not found in this folder."
  echo "Put start-bot.sh next to the shaded JAR from mvn package (target/evrima-server-bot-1.0.2.jar)."
  exit 1
fi
